...
static int32_t pi_update(struct feedback_ctx *ctx)
{
	 ...
	 * The control loop performance was checked with nRF54H20 with all the
	 * above mentioned hosts, using a wide frequency range achieved by
	 * deliberately misconfiguring AudioPLL clock to achieve I2S FSYNC
	 * frequency in range 47.1 kHz up to 48.9 kHz with a 1 Hz step. When the
	 * steps were limited to ~3 Hz per second then the "steady state" error
	 * (during frequency ramp) was 1.5 sample at High-Speed and 5 samples
	 * at Full-Speed. When the frequency was stable, then regardless of what
	 * value it was, there was no steady state error.
	 *
	 ...
	 * stop I2S streaming). This regulator is able to stabilize without
	 * buffer underrun for any frequency that is within required USB SOF
	 * accuracy of 500 ppm, i.e. when nominal sample rate is 48 kHz the real
	 * sample rate can be anywhere in [47.976 kHz; 48.024 kHz] range.
	 */
	ctx->integrator += error;
	return (error + (ctx->integrator / (ctx->high_speed ? 8192 : 2048))) / 32;
}
